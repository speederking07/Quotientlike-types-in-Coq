...
| S' : conat -> conat.
\end{minted}
In contrast to an inductive definition of natural numbers, co-natural numbers are coinductive. That means we define destructors instead of constructors. Destructors describe that from a co-natural number, we can either get \mcoq{O'} -- zero or \mcoq{S' n} -- the successor of co-natural number n. Since we use destructors, we can define a number that is a successor of itself.
\begin{minted}{coq}
...
\begin{func}{Addition to a set}{fn:set_add}
\begin{minted}{coq}
...
\end{minted}
\end{func}
Proof of fundamental properties of those operations can be found in \coqsource{FunctionalQuotient.v}.