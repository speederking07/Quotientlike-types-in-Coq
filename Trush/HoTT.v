...
Theorem eqDec_pathdec (A: Type) `{EqDec A} : DecidableEq A.
Proof.
  intros x y. case_eq (eqf x y).
  - intro H1. left. apply eqf_iff; auto.
  - intro H1. right. intro H2. subst. rewrite eqf_refl in H1. inversion H1.
Qed.
...
Theorem pathdec_eqDec {A: Type} `{DecidableEq A} : EqDec A.
Proof.
  unfold DecidableEq in H. exists (fun x y => isLeft (H x y)).
  intros x y. destruct (H x y); cbn; constructor; auto.
Qed.

Theorem dec_eq_hset {A: Type} `{EqDec A} : isHSet A.
Proof.
  apply pathdec_hset. apply eqDec_pathdec. assumption.
Qed.
